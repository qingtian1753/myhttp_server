...
void EventLoop::handleRequest(int fd, const HttpRequest& req)
{
    ...
    client->enqueueResponse(HttpResponse::buildJsonResponse(400, "Bad Request", "{\"code\":1,\"msg\":\"invalid request\"}", client->keepAlive));
    epoll_.modEpollToWrite(fd);
}
...
void EventLoop::handleStaticFile(HttpConnection& client, const HttpRequest& req)
{
    ...
    if(req.method == "GET" && filePath == "./www/index.html" && !route::checkIndex(req))
    {
